...
export function FeatureCard({
  ...
  return (
    <div
      className={cn(
        "group relative overflow-hidden rounded-md sm:rounded-lg md:rounded-xl border border-(--color-border-default) bg-(--color-bg-surface)/50 p-1 sm:p-2 md:p-3 text-center",
    ...
    >
      ...
      <div className="relative font-medium text-(--color-text-primary) text-[8px] sm:text-[11px] md:text-xs leading-tight pb-0.5">
        {name}
      </div>
      {description && (
        <div className="relative mt-0 sm:mt-1 text-[7px] sm:text-[9px] md:text-[11px] text-(--color-text-muted) leading-relaxed">
          {description}
        </div>
      )}
    </div>
